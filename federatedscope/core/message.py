...
class Message(object):
    ...
    def __init__(self,
                 ...
                 strategy=None,
                 serial_num=0):
        self._msg_type = msg_type # 메시지 종류 ('train', 'eval', 'model_para' 등)
        self._sender = sender # 송신자 ID
        self._receiver = receiver # 수신자 ID
        self._state = state # 라운드 번호
        self._content = content # 메시지 내용 (모델 파라미터 등)
        self._timestamp = timestamp # 해당 메시지가 생성된 시점" 을 나타내는 숫자 (float 또는 int). FL에서 여러 클라이언트가 동시에 메시지를 보낼 수 있으므로, 서버는 메시지의 도착 순서나 처리 순서를 정할 기준이 필요. "논리적 시간 스탬프" 역할을 하며, 일반적으로 현재 라운드 번호나 시스템 내의 time.time() 값 등으로 설정
        self._strategy = strategy #선택 전략 등 추가 필드 (일반적으로 생략 가능)
        self.serial_num = serial_num #같은 라운드, 같은 타임스탬프를 가진 메시지 사이에서 우선순위를 정하기 위한 고유 번호. 일반적으로 메시지가 생성될 때 서버나 클라이언트에서 자동으로 증가시켜 부여
        self.param_serializer = b64serializer #파라미터 직렬화 방식 (기본은 b64serializer)
