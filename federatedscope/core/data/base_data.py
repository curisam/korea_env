...
#단일 프로세스 모드에서 전체 서버·클라이언트 데이터를 한데 묶어 Runner 에 넘겨주기 전에, 
# 글로벌 설정에 따라 전처리(병합·공격 주입 등)를 수행하는, dict 기반의 컨테이너    
    """
    ...
    """
    def __init__(self, datadict, global_cfg): #datadict: {0: ClientData(서버), 1: ClientData(클라이언트1), …}
        """
        ...
        Args:
            ...
            global_cfg: global CfgNode
        """
        self.global_cfg = global_cfg   # 전체(run) 설정 보관
        self.client_cfgs = None        # 클라이언트별 별도 설정(나중에 HPO 등에서 사용)
        datadict = self.preprocess(datadict)  # 병합·공격 등 전처리
        super().__init__(datadict)     # dict로 초기화
    ...
    def setup(self, new_client_cfg=None):
        ...
        for split_data, split_name in zip(
            ...
            if split_data is not None:
                ...
                if isinstance(split_data, csc_matrix):## (a) 희소행렬인 csc_matrix는 __len__이 없으니 예외처리
                    self[split_name] = get_dataloader(split_data,
                                                      self.client_cfg,
                                                      split_name)
                elif len(split_data) > 0:# (b) 일반 Dataset/list는 길이가 0이 아닐 때만
                    self[split_name] = get_dataloader(split_data,
                                                      self.client_cfg,
                                                      split_name)
